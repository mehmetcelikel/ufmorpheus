...
%Once we have the categories belonging to a particular realm, we associate them with WordNet synsets using an approach similar to YAGO. We determine a \emph{head noun} in the category name and search for it in the WordNet synsets. DBpedia categories having no WordNet synset match are excluded from our ontology. <--- by Gordon
...
Suppose we want to find the category divergence between \textit{Bus} and \textit{Sedan} (Figure \ref{fig:vehicular_ontology}). \textit{Sedan} is a subcategory of \textit{Automobile}, and \textit{Bus} is a subcategory of \textit{Land\_Vechicle}. Therefore, the least common ancestor ($C$) for these two is \textit{Land\_Vechicle}. In addition, the tree height \textit{h = 4}. So, the normalized divergence $cd$\textit{(Bus, Sedan)} is \textit{0.5} that is calculated from \textit{d(Bus, Root) = 3}, \textit{d(Bus, Land\_Vehicle) = 1}, and \textit{d (Sedan, Land\_Vehicle) = 2}.
