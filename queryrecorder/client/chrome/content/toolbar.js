...
var fileCounter = 0;
var dirCounter = 0;
var curDirName = null; //This is the query id
var queryData = null; //This variable contains the query and the realm
var form_defaults = null;
var recording = false; // thsi variable keep track if the application is recording or not
...
/*
	Starts the recording process by getting the query and the realm
	TODO: get the query id from the database
*/
function start() {
	...
	var query = { param1: null, 
				param2: null, 
				};
	window.openDialog("chrome://mcr/content/start.xul","Start Recording","chrome,modal",
		query); // [0]

	queryData = query;
	
	var name = query.realm + "-" + query.name.replace("?","");
	
	//curDirName = query.realm + "_" + query.name.split(' ').join('_')
	curDirName = name.split(' ').join('_');
	
	//alert("The directory is " + curDirName);	
...
function stopRecording() {
	if(recording) {
		window.openDialog("chrome://mcr/content/stop.xul","Stop Recording","chrome,modal", queryData, fileCounter, curDirName); 	
...
/*
	...
	Changes in old code	
	*Added if statment on SaveSelected() 
...
/*
	...
	Guillermo changes: changed the name of MCR_SaveDataToFile(fileCounter + "-" + MCR_GenId(), data); to just fileCounter
*/
function MCR_SaveData() {
	var data = MCR_GenPageInfo();
	//MCR_SaveDataToFile(fileCounter + "-" + MCR_GenId(), data);
	MCR_SaveDataToFile(fileCounter, data);
	fileCounter++;
}
...
/*
	Guillermo:Added if statment for recording
*/
function SaveSelected() {
	if(recording) {
		...
		//MCR_SaveDataToFile(fileCounter + "-selection-" + MCR_GenId(), data); old one
		MCR_SaveDataToFile(fileCounter, data);
		fileCounter++;
	}
